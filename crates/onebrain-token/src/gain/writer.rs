...
impl JsonlGainWriter {
    ...
    /// Reads every well-formed event across all monthly files in the
    /// directory, oldest file first. A truncated or corrupt final line in
    /// any file (the crash-tolerance case) is skipped, not an error.
    pub fn read_all(&self) -> io::Result<Vec<GainEvent>> {
        ...
        for path in files {
            ...
            for line in io::BufReader::new(file).lines() {
                ...
                // Malformed/partial line (e.g. crash mid-write): skip it,
                // don't fail the whole read.
            }
        }
