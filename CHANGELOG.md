...
- Internal robustness: the auto-started daemon receives its vault via a `daemon start --vault` argument (no `std::env::set_var`), and a vault that can't be canonicalized is treated as a conservative mismatch — so a client never routes to a wrong-vault daemon.
