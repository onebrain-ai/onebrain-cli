...
### Added
- **MCP server routes through the warm daemon.** `onebrain mcp` obtains a daemon via `daemon_client::ensure_running()` at startup and routes its engine-backed tools (`status`, `query`) through it (`GET /api/internal/status` + `GET /api/vault/search`) instead of opening its own `Engine` — so it no longer holds redb's exclusive lock and **multiple concurrent MCP sessions coexist**. `get`/`multi_get` stay filesystem reads. Falls back to a direct single-process engine open if the daemon can't start (a lone session still works), and exports `$ONEBRAIN_VAULT` so an auto-started daemon serves the same vault. (#168)
