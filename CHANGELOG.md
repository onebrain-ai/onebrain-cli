...
### Added
- **Tier-2 cross-encoder reranker, default-on.** After hybrid retrieval, `query`/`vsearch` cross-encode the top fused candidates with `onebrain-rerank-v1` (the first OneBrain model — bge-reranker-v2-m3 int8, self-hosted + sha256-pinned) and re-rank by a calibrated 0–1 score, then gate at `min_score` with a never-empty floor (keeps the top 3 labelled "no strong match" if nothing clears it). Its calibrated score replaces ADR 0024's cosine heuristic as the confidence gate — real matches score 0.73–0.99 vs no-answer ≤0.07, an order of magnitude cleaner than e5's overlapping cosine. CLI confidence bands + honest "unreranked" hint when the model isn't downloaded; `search model list`/`remove`, `status`, `doctor`, and daemon `/api/internal/status` all report reranker health. Skip-not-fail: a missing model, lex-only build, or rerank error falls back to fused order — a query never fails because reranking did. ([ADR 0025](docs/decisions/0025-tier2-cross-encoder-reranker.md)) (#190, #191)
- **`search.reranker` config** — `enabled` (default true), `model`, `min_candidates` (default 10, calibrated on the real vault; raise for a deeper reranked pool), `min_score` (default 0.30). The model downloads during `reindex` (not a surprise query-time stall) and is sha256-verified on load.
- **top_k and reranker min_candidates are settable on every surface** (CLI `--min-candidates` / `--top-k`, `search.default_top_k` + `search.reranker.min_candidates` config, and `top_k`/`min_candidates` query params on `/api/vault/search`); the reranker now always covers the full returned set (`min_candidates` is a floor, auto-raised to `top_k`), so the web service reranks all results instead of just the first 10.
- **`--min-score` is unified with the rerank gate** — when reranking is active it filters the calibrated 0–1 `rerank_score` (a confidence threshold a user can reason about) instead of the opaque raw retrieval score; with reranking off (disabled / model absent / pure-lex `search`) it keeps its legacy raw-score meaning.
- **MCP `query` tool reranks + surfaces `rerank_score`.** The multi-sub-query fusion tool (the surface Claude Desktop uses) now applies the Tier-2 rerank stage **after** its command-layer RRF fusion, against the primary sub-query text, so its results are reranked and carry `rerank_score` like every other surface. Direct backend reranks via the engine; daemon backend reranks through the new token-guarded `POST /api/internal/rerank` (model stays single-owner in the warm daemon). Skip-not-fail: a rerank error keeps the fused order with no `rerank_score`.

### Fixed
- **Reranker model could never download** (pre-release, caught by the real-vault gate): the reindex fetch asked the query-path accessor to fetch, but that accessor skips construction when the model is absent — so the model could never arrive and the reranker was inert for every real user. reindex now downloads via a dedicated path. A network-gated end-to-end test now covers the cross-task cold path the unit tests structurally could not.

## [3.4.6] — warm daemon + honest search-engine lock contention

### Added
- **Warm daemon owns the native-search engine.** `daemon __run` opens the engine once and holds it (the sole redb owner), so mcp + CLI search become clients instead of each racing redb's single-writer lock. Token-gated `POST /api/internal/reindex` (`pending`/`paths`/`lex`/`all`) + `GET /api/internal/{status,get}` + engine-independent `GET /api/health`; `~/.onebrain/run/daemon.json` discovery (with the daemon's bound-vault identity), a cross-platform `O_EXCL` concurrent-start guard (N starts → one daemon; `daemon stop` clears a wedged lock), idle-shutdown TTL (`$ONEBRAIN_DAEMON_IDLE_SECS`), and a `daemon_client` lib (health discovery · auto-start with retry-once · version/vault-skew restart). Reindex `paths` are vault-confined (rejects `..`/absolute/symlink-escape); `serve` unchanged. ([ADR 0023](docs/decisions/0023-warm-daemon-mcp-search.md) · [docs/daemon.md](docs/daemon.md)) (#164)
- **`onebrain mcp` and CLI search route through the daemon** instead of each opening their own engine, so **multiple concurrent sessions coexist** and the CLI keeps working while a session holds the engine (auto-reindex lands **in-session**). One vault at a time per machine: the mcp path restarts a wrong-vault daemon, the CLI path uses a **passive** `discover_matching` that routes-direct (never starts/restarts) so a `search` can't disrupt another vault's live session. No daemon / wrong vault / `ONEBRAIN_NO_DAEMON=1` → direct engine open (concurrent per-vault daemons deferred to v3.8). (#168, #169)

### Fixed
- **Honest, uniform search-engine lock contention.** While `onebrain mcp` holds the index, other commands now surface the busy state instead of misreporting (classified from redb's typed `DatabaseAlreadyOpen`): user verbs → `E_ENGINE_BUSY` + exit **77**; hooks (`--lex-only`/`--pending-only`) skip with `reason:"engine-busy"` + exit 0 (never break the Claude Code hook chain); `search status` reports `busy`/`status_error` + `doc_count:null` and **never** shows "✅ up to date" over a locked or broken/unreadable index (underlying error logged to stderr, never swallowed).
- `search search` (lex verb) populates each hit's `heading_path` from the STORED tantivy field in a single search pass (no per-hit re-query; snippet deferred pending a `body`-STORED schema change).
- Internal robustness: the auto-started daemon receives its vault via a `daemon start --vault` argument (no `std::env::set_var`), and a vault that can't be canonicalized is treated as a conservative mismatch — so a client never routes to a wrong-vault daemon. Reindex-path confinement now also runs in the **engine** (`reindex_paths` rejects `..`/absolute/symlink-escape as "outside the vault" and indexes nothing), so the DIRECT path (`ONEBRAIN_NO_DAEMON=1`, daemon fallback, plain `search reindex <paths>`) is protected too — not just the HTTP layer. (#175)
- **Honest errors during the upgrade-transition window.** When a pre-upgrade `onebrain mcp` still holds the redb engine while the new warm daemon runs, the daemon is engine-less and its endpoints 503. `query`/`get`/`reindex` now report `E_ENGINE_BUSY` (exit 77) and `status` shows `busy:true` + `doc_count:null` — instead of leaking an opaque `E_INTERNAL` / raw `503` string. `/api/vault/search` now signals engine-busy as **503** (matching `/api/internal/*`), and `daemon_client` classifies a 503 as the typed `EngineBusy`, so the routed path reports contention identically to the direct open. (#179)
- **Semantic search no longer silently returns nothing.** The per-model absolute vector floor (0.85 for e5) straddled genuine e5 match scores (~0.83–0.87, overlapping the ~0.84 unrelated baseline), so `vsearch` returned 0 and `query` (hybrid) silently degraded to keyword-only. Replaced with a recall-first, model-agnostic cutoff — `keep_top_cluster` (keep the top cluster within 0.02 of the best score; never empties a non-empty set) — plus a **never-silent** confidence label on weak `vsearch` results and RRF that keeps vec's contribution. Per-model `vec_floor` removed. A cross-encoder reranker (the real precision fix) follows in a later v3.4.x. ([ADR 0024](docs/decisions/0024-vector-confidence-recall-first.md)) (#183)
...
- Removed the `onebrain qmd …` command group and the runtime dependency on the
  ...
  - Note: the reindex-on-write hook now runs **synchronously** (native `search
    reindex` is foreground; the old `qmd` target spawned a detached background
    process). The reindex is hash-diff incremental, so steady-state cost is a
    tree walk plus embedding only changed chunks — moving it back to a detached
    / debounced spawn is tracked in the auto-reindex hook work (#133).

- **Native-search state (embedding model + tantivy/vector index + `engine.redb`) now lives in the OS data dir** — `~/Library/Application Support/onebrain/search/` (macOS) · `$XDG_DATA_HOME/onebrain/search/` (Linux) · `%APPDATA%\onebrain\search\` (Windows) — instead of the OS-**purgeable** cache dir, after macOS storage cleanup silently wiped a ~536 MB model + index from `~/Library/Caches` (#114); expensive-to-recreate data must not live somewhere the OS may evict. Existing state migrates automatically and transparently on the next command (atomic same-volume rename `<cache>/onebrain/search` → `<data>/onebrain/search`, no re-download / re-embed, best-effort with a stderr warning on failure); only the `search/` subtree moves — the disposable `latest-release.json` update-check cache stays in the cache dir by design. See [ADR 0021](docs/decisions/0021-search-state-persistent-data-dir.md).
- **Honest index-state reporting after a purge.** `doctor` flags a missing index on an already-configured collection as a possible OS storage-cleanup purge (pointing at `onebrain search reindex`) instead of the neutral "no index yet"; `search status`'s `indexed` now derives from actual doc count (`doc_count > 0`), not mere cache-dir existence, so it agrees with the MCP `status` tool. **No-model reindex UX**: `search model list` / the interactive TUI only mark a model active when it's actually downloaded, clear the stale `search.embed_model` key on a purged vault so the first-run picker re-fires, and `search status` gains a `current_model_missing` flag + hint. **MCP `query` degrades cleanly with no index** — returns empty results plus a fallback-to-grep `note` instead of a cryptic index-open error.
- **Auto reindex/embed hook** — `search reindex --lex-only` runs on PostToolUse (incremental keyword pass, no model load) and `search reindex --pending-only` runs on Stop hook (background embed of pending docs); both skip silently when search isn't set up. Existing hook entries auto-migrate on the next `onebrain plugin update` / `onebrain register hooks` (#133). The hook paths' live-progress marker now reads the already-resolved (read-only) collection cache dir instead of re-resolving through the persisting resolver, closing the last gap in the hooks' "never mutates config" contract.
- **Session-init field `qmd_unembedded` → `search_unembedded` (transition).** `onebrain session init --json` now emits the canonical `search_unembedded` key **alongside** the deprecated `qmd_unembedded` alias with the same value for one transition version.
- **Internal cleanup:** renamed `HookSpec::QMD` → `REINDEX` (the PostToolUse hook has run native `search reindex --lex-only` since Track 2, so `qmd` was a misnomer; fixed a stale version note in `docs/reference/mcp.md`) and removed the dead init-only `.obsidian/` seeding path (`vault_sync::sync_obsidian` + `include_obsidian`, never set in production — pure cleanup, no behavior change). Closes onebrain-ai/onebrain-cli#142.
- **fix: `plugin update` on a vault with no `update_channel` no longer 404s.** `resolve_branch` defaulted an absent/unknown channel to the `next` branch, which does not exist in the plugin repo (only `main`), so the tarball fetch failed. It now defaults to `main`; only `update_channel: next` resolves to `next` (reserved for a future pre-release branch). `stable` still resolves to `main`.
...
- **Generated plists now use the current `skill run` subcommand** instead of the deprecated `run-skill` alias, so scheduled runs no longer log a v3.1 deprecation notice on every fire (#125). Existing schedules pick this up on the next `onebrain schedule register`.
...
- **Locked 27-entry command tree** — 3 root verbs (`init`/`update`/`doctor`) + 24 resource groups, all paths singular-noun 2-level `onebrain <noun> <verb>`. Working verbs in v3.1: `session init`, `checkpoint stop/reset/orphans`, `qmd reindex`, `vault sync/current`, `harness detect`, `plugin install/update/migrate`, `schedule register`, `skill run`. Other 200+ verbs stubbed with stable `E_NOT_IMPLEMENTED` exit 72 — tree shape locked for v3.2+. `onebrain --help` shows only the visible surface (stub-only groups + stub verbs hidden, domain-clustered ordering, one-line `about` per group, dev-log preamble stripped) — typed stub commands still dispatch. Hidden v3.0 aliases (`session-init`, `orphan-scan`, `qmd-reindex`, `register-hooks`, `register-schedule`, `migrate`, `vault-sync`, `run-skill`) still work, each printing a one-time migration notice (suppressible via `ONEBRAIN_QUIET_MIGRATION=1`, state in `~/.cache/onebrain/migration-shown.txt`) before dispatching to the v3.1 handler; removal is a future major, none scheduled for v3.x.
- **`--vault` global flag + walk-up resolver + `ONEBRAIN_VAULT` env** — documented priority `--vault <PATH>` > `ONEBRAIN_VAULT` env > walk-up from cwd, encoded in `onebrain_core::path::resolve_vault` and surfaced by the new `onebrain vault current` verb (reports which mechanism resolved the vault). `onebrain init` drops the redundant `--vault-dir` in favor of this global flag (hidden v3.0 aliases keep `--vault-dir` through v3.5; on `init`, `--vault PATH` always means "create vault at PATH").
...
- **Fix: `onebrain init` now registers the plugin with Claude Code AND prompts before initializing in a non-empty directory** — fresh init writes `<vault>/.claude-plugin/marketplace.json` AND merges `enabledPlugins.onebrain@onebrain = true` into `.claude/settings.json` (idempotent). Classifies the target (missing → create / empty → proceed / has vault.yml → defer to overwrite guard / non-empty non-vault → require confirmation); `--yes` without `--force` fails closed; structured-output modes emit canonical `E_INIT_TARGET_NOT_EMPTY` exit 75 envelope without prompting (also the code for a non-bool `enabledPlugins.onebrain@onebrain` refused without `--force`). Round-4 hardening: marketplace.json write failure now returns Err; `--force` re-init validates/rewrites malformed manifest shape via atomic tmp+rename and no longer bypasses `safety::classify` (EACCES / target-is-a-file surface as proper FsError before partial writes). Vaults init'd before this fix need one `onebrain init --force` to pick up the registration.
- **Canonical `Envelope<T>` JSON shape + partial-failure contract (`E_PLUGIN_UPDATE_PARTIAL`) + `BrokenPipe` → exit 0.** Every `--json` output wraps payloads in `{version, command, ok, vault, data, warnings, error}` (skill-alignment §4.3; `vault`/`error` omitted when `None`, `warnings` always `[]`); `--yaml` emits the same shape, `--output {text,json,yaml,table,tsv}` matrix mandatory on every command. Long-running multi-step commands (plugin update, vault sync, schedule register) that complete part of their work before failing emit `Envelope<T>::partial` with `ok = false`, `error.code = E_PLUGIN_UPDATE_PARTIAL`, and the completed-state snapshot in `data`; exit code 1 distinguishes partial from total failure across JSON/YAML/table/TSV. Writes to closed-pipe stdout (e.g. `onebrain doctor --json | grep`) now exit cleanly with code 0 instead of a Rust panic — the dispatcher catches `io::ErrorKind::BrokenPipe` on every envelope-write surface.
- **Output-format compliance · default = text · explicit flag = honored** — interactive commands now default to human-readable text and honor `--json` (compact) · `--json --pretty` (indented) · `--yaml` · `--output {json,yaml}` consistently across `session init`, `checkpoint orphans`, `harness`, `doctor`, `update`, fixing v3.1.0-rc audit regressions (`harness` emitted JSON unconditionally; `doctor --yaml` returned the JSON envelope; `update --yaml` silently fell back to text) via one canonical `serialize_for_mode` dispatcher. Two hook-only commands (`checkpoint stop`, `qmd reindex`) still hard-wire JSON (fixed scope, harmless — hook-only callers). Covered by a parametric test matrix (`tests/output_format_matrix.rs`, `tests/user_flows.rs`) and pre-tag QA artifacts (`scripts/smoke-ux.sh`, `docs/release-checklist.md`).
- **Breaking: config file renamed `vault.yml` → `onebrain.yml`** — `onebrain init` writes the canonical `onebrain.yml` going forward. CLI v3.1+ dual-reads for back-compat (prefer `onebrain.yml`, fallback `vault.yml` with a one-time `W_VAULT_YML_DEPRECATED` stderr warning, suppressible via `ONEBRAIN_QUIET_VAULT_YML_DEPRECATION=1`). New `vault-config-migration` doctor check fires 🟡 when legacy `vault.yml` is detected; `onebrain doctor --fix` performs a single atomic `fs::rename` to migrate. v4.0.0 will drop `vault.yml` support entirely.
