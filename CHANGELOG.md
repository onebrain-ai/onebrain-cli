...
- **`search.reranker` config** — `enabled` (default true), `model`, `candidates` (default 10, calibrated on the real vault; raise for deeper reranking), `min_score` (default 0.30). The model downloads during `reindex` (not a surprise query-time stall) and is sha256-verified on load.
