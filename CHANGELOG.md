...
### Added
- **MCP server routes through the warm daemon.** `onebrain mcp` obtains a daemon via `daemon_client::ensure_running()` and routes its engine-backed tools (`status`, `query`) through it (`GET /api/internal/status` + `GET /api/vault/search`) instead of opening its own `Engine` — so it no longer holds redb's exclusive lock and **multiple concurrent MCP sessions coexist**. `get`/`multi_get` stay filesystem reads; falls back to a direct engine open if the daemon can't start. The daemon now records its bound vault (canonical path) in `daemon.json`, and clients **restart a daemon bound to a different vault** — one vault at a time per machine; switching vaults restarts the daemon (concurrent per-vault daemons deferred to v3.8). Daemon-routed `query` is **doc-level** (dedup by `path`), serves `vec` as `hybrid`, and caps at `TOP_K`=20 per sub-query (`hyde`≡`vec` on both paths; `status` shape identical). (#168)
- **Warm daemon owns the native-search engine.** `daemon __run` now opens the search engine ONCE at boot and holds it for the process lifetime (the sole redb owner), so mcp + CLI search can be clients instead of each opening their own engine and racing redb's single-writer lock. New token-gated `POST /api/internal/reindex` (`pending`/`paths`), `GET /api/internal/status`, and engine-independent `GET /api/health` endpoints; `GET /api/vault/search` reuses the held engine. Reindex `paths` are confined to the vault at the HTTP layer (rejects `..`/absolute/symlink-escape/tooling paths). Adds a `~/.onebrain/run/daemon.json` discovery file (`port`/`token`/`pid`/`version`), a self-describing cross-platform `O_EXCL` concurrent-start guard (N starts → exactly one daemon; `daemon stop` clears a wedged lock), an idle-shutdown TTL (`$ONEBRAIN_DAEMON_IDLE_SECS`, default 30 min), and a reusable client library (`daemon_client`) with health-based discovery, auto-start (retry-once on transport failure), and version-skew restart. `serve` is unchanged (still per-request). See [ADR 0023](docs/decisions/0023-warm-daemon-mcp-search.md) and [docs/daemon.md](docs/daemon.md). (#164)
