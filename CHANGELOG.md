...
- **No-model reindex UX.** `search model list` / the interactive `search model` TUI now show the active marker (●/green) only for the chosen model when it's actually downloaded — a configured-but-purged model reads as "no active model" (with `multilingual-e5-small` as the default) and points the user at `search model` / `search reindex`. Opening the TUI or running `search reindex` on a purged vault clears the stale `search.embed_model` key so the first-run pick re-fires (TTY → interactive picker; headless → the e5-small default, no prompt). `search status` gains a machine-readable `current_model_missing` flag plus a "no embedding model downloaded — run `onebrain search reindex`" hint (suppressed while a reindex is running).
...
- **Session-init field `qmd_unembedded` → `search_unembedded` (transition).** `onebrain session init --json` now emits the canonical `search_unembedded` key **alongside** the deprecated `qmd_unembedded` alias with the same value, so consumers reading either key keep working for one transition version. The OneBrain plugin adopts `search_unembedded` (with a legacy fallback) next; the alias is dropped afterward.
- **Renamed internal hook spec `HookSpec::QMD` → `REINDEX`** — the PostToolUse hook has run native `search reindex --lex-only` since Track 2, so the `qmd` name was a misnomer. Legacy `qmd …` entry detection/migration is unchanged (it matches real command strings in existing `settings.json`). Also fixed a stale version note in `docs/reference/mcp.md` (the plugin `.mcp.json` key rename is v3.4.5, not "v3.4.4").
- **Removed the dead init-only `.obsidian/` seeding path** (`vault_sync::sync_obsidian` + the `include_obsidian` option): the option was never set in production, so the path was unreachable — a pure cleanup, no behavior change. Closes onebrain-ai/onebrain-cli#142.
