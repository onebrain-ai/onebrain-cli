...
| **Runtime** (hook protocol) | `hook`, `session init`, `checkpoint stop · reset · orphans`, `search reindex · search reindex --lex-only · search reindex --pending-only` | Harness registrations call only `onebrain hook`; stdin `hook_event_name` dispatches SessionStart to initialization, PostToolUse/AfterTool to lexical reindex, and Stop/AfterAgent to checkpoint plus pending reindex. The listed checkpoint/search commands remain valid direct child/user commands. The runner emits hook JSON and suppresses the banner for clean machine stdio. |
