...
- **Per-artifact fallback resolution for every read-only consumer** (`search status`, `doctor`, hook gates, MCP/daemon lex paths, the model TUI, `session init`): new location if present, else legacy root — and **never `migrate()`**. This is per *artifact*, and for models per *model dir* (`model_dir(cache_dir_name)`): a "split-brain" model cache (model A still legacy at the root, model B already under `models/`) resolves each model to where it actually lives. A single shared probe base cannot represent that state — an earlier draft that fell back to the root "if any legacy model remains" misreported migrated models as not-downloaded (re-download risk) and was replaced by per-model resolution.
