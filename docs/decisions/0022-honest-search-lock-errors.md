...
- The lex `search` verb's `heading_path` is real and now filled in a single search pass (`search_with_heading` reads `chunk_id` + `heading_path` from the same retrieved doc — no per-hit re-fetch). Only the `snippet` remains deferred follow-up work, folded into the future body-STORED schema change + reindex migration. This ADR deliberately does NOT claim snippets ship in v3.4.6.
