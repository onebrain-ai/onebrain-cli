...
- Existing users migrate transparently on their next command — no manual step, no re-download, no re-embed — because the move is a same-volume rename (`~/Library/Caches` and `~/Library/Application Support` share a volume; likewise `~/.cache`↔`~/.local/share` and `%LOCALAPPDATA%`↔`%APPDATA%`), which is atomic and instant regardless of index size.
