...
- **Thai handled honestly.** tantivy's default tokenizer can't segment space-less Thai, so a custom tokenizer routes Thai runs through `nlpo3` (dictionary newmm); semantic Thai is additionally carried by the multilingual embeddings.
