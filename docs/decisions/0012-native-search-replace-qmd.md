...
- **Markdown-only index.** The vault walk collects only `*.md` files; every other file type (images, PDFs, code, …) is never indexed. `search reindex`, `search status`, and the query verbs all operate on this `.md`-only corpus.
