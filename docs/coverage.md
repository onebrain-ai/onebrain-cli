...
- **`capture_note`'s best-effort reindex block (Gateway PR 4, Task 5) joins the same subprocess-capture gap** — `spawn_blocking(move || match daemon_client::ensure_running(...) { ... })` (`server.rs`, inside `capture_note`) shows 0% under `cargo llvm-cov` for the identical reason `brain_search`'s daemon body does: it only executes for real inside `tests/gateway_approval_e2e.rs`'s spawned `onebrain gateway run` subprocess (confirmed empirically — that test leaves a real `onebrain daemon __run` process running against its tempdir vault after the run), and a spawned child's `.profraw` data never merges back into this report. The in-process `fixture_router_with_mutating_policy`-based `brain_capture_*` tests in `server.rs`'s own test module never reach a real daemon (no `$HOME`/cache sandbox is set up for them), so they cannot cover this block either — that's a deliberate, not accidental, gap: those tests exist to drive the policy/approval state machine cheaply and fast, not to exercise the daemon.
