...
`crates/onebrain-cli/src/commands/search_common.rs` — the Track 2c routing helpers are core (not excluded): `daemon_routing_disabled` and `route_to_daemon`'s disabled / no-daemon / wrong-vault-goes-direct-and-leaves-daemon-untouched arms are unit-tested (the vault-match itself is #170's `vault_decision`/`canonical_vault_id`, unit-tested in `daemon_client`). Only `route_to_daemon`'s same-vault-live routing needs a running daemon and is covered via `discover_matching`'s live-server test.
