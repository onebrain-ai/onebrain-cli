...
- **`pair()` (Task 6, `onebrain gateway pair [--rotate]`) inherits the exact same capture gap** — it too only executes as the real compiled binary, spawned by `tests/gateway_oauth_e2e.rs::gateway_pair_verb_prints_and_rotates_the_pairing_code` (which asserts the printed line and the persisted `pairing.json` code match on a plain call, are idempotent across repeated plain calls, and diverge after `--rotate`). This is why `commands/gateway/mod.rs`'s file-level line % dropped further (39.50% at last measurement, `run()` alone) — both host functions are proven correct end-to-end, neither is capturable by `cargo-llvm-cov` through a subprocess boundary.
...
- **`capture_note`'s best-effort reindex block (Gateway PR 4, Task 5) is now uncovered BY CONSTRUCTION, not by a capture gap** — `spawn_blocking(move || match daemon_client::ensure_running(...) { ... })` (`server.rs`, inside `capture_note`) shows 0% because the fix wave put it behind `reindex_channel_enabled()`, which is `false` in every test that could reach it: `cfg!(test)` closes it for this crate's own unit tests, and `ONEBRAIN_GATEWAY_DISABLE_DAEMON_REINDEX=1` closes it for `tests/gateway_approval_e2e.rs`'s spawned gateway subprocess. That is deliberate — the block spawns `onebrain daemon start`, and an earlier revision of that e2e genuinely left a live `onebrain daemon __run` process behind on the developer machine after every run. Both closures are asserted, not assumed: `reindex_channel_is_disabled_inside_this_test_binary` and `a_unit_test_capture_leaves_no_daemon_runtime_state_under_home` in-process, and the e2e's own `$HOME/.onebrain/run/`-must-not-exist assertion out-of-process. The `ensure_running`/`reindex` call it wraps is itself already documented as residual above (`daemon_client.rs`), and `brain_search`'s identical daemon body remains covered functionally by `tests/gateway_http.rs`, which deliberately DOES drive a real daemon.

#### Gateway approval flow (v3.5, Gateway PR 4, Task 6 — NOT whole-file-excluded)

`crates/onebrain-cli/src/commands/gateway/approval_native.rs`'s `run_dialog` (the actual `Command::new("osascript").arg("-e").arg(script).output()` call and its success/failure branches) and `prompt`'s entire body (`is_available()` check through the `spawn_blocking` closure that calls `run_dialog`) are 0% covered on this macOS measurement, by design, not oversight:

- Every in-process unit test that drives a `NeedApproval` call through `server::await_approval` runs with `cfg!(test)` true, which short-circuits the `if !cfg!(test) && approval_native::is_available() { approval_native::prompt(...) }` call site in `await_approval` before `prompt` is ever reached — the guard `approval_native.rs`'s own module docs (and Gateway PR 4, Task 5's report) call out to keep this crate's own test run from popping a real GUI dialog.
- `tests/gateway_approval_e2e.rs` (this task) is the first test to spawn the real binary and drive `brain_capture` under a real `ask_once` policy — the ONE thing that could give `prompt` a real caller outside `cfg!(test)`. It deliberately keeps this path closed anyway: it sets `ONEBRAIN_GATEWAY_DISABLE_NATIVE_APPROVAL=1` in the spawned gateway's environment (the BINDING requirement for that test — see its own module doc comment), which makes `approval_native::is_available()` return `false`, so `await_approval`'s `&&` short-circuits before `prompt` — same outcome as the `cfg!(test)` guard, different mechanism, same net effect: no test anywhere in this repository has ever invoked a real `osascript display dialog`, and none should.
- `run_dialog`/`prompt`'s CORRECTNESS is still fully covered indirectly: `build_dialog_script` (the AppleScript text `run_dialog` would hand to `osascript`, including every escaping property) and `decision_from_button_output` (the parse of `osascript`'s stdout `run_dialog` would perform) are both unit-tested directly and exhaustively — the only untested seam is the literal `Command::new("osascript")...output()` I/O call gluing the two together, which cannot be exercised without a live GUI session.
