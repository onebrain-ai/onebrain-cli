...
Implements `skill run` (and the `run-skill` alias). Verifies a config file exists (else exit 78), parses `--arg key=value` pairs, builds the prompt (`onebrain_fs::build_prompt`), resolves the claude binary (`onebrain_fs::resolve_claude_bin`), spawns `claude -p "<prompt>" --add-dir <vault>` with inherited env/stdio. Exit codes mirror Bun (78/127/128+signal/child code). Library: `onebrain_core::find_config_file`, `onebrain_fs`.
