...
- `is_hook_protocol(cmd) -> bool` — true for `session init`, `session token`, all `checkpoint *`, and their hidden aliases (incl. the legacy `qmd-reindex` alias, which now dispatches to native `search reindex` but stays banner-suppressed for un-migrated hooks); plain `search reindex` is not part of this gate.
...
Implements `session init` (hook-protocol; SessionStart hook). Walks up for the vault, loads config, resolves the session token, and — only when the vault has a `search.collection` configured (falling back to the legacy `qmd_collection` key) — probes the **native** search index for the pending/unembedded doc count via `native_pending_bounded` (a time-boxed wrapper over `native_pending`, which checks the collection's cache dir exists via `commands::search_common::{collection_cache_dir, is_indexed}` before opening `onebrain_search::engine::Engine` and calling `.status(..).pending_total()`); no `qmd` subprocess is spawned. Emits `SessionInitOutput` (still carrying the `qmd_unembedded` field name for wire back-compat) or a `SessionInitBlock`. Library calls: `onebrain_core::{find_vault_root, load_vault_config}`, `onebrain_cache::{resolve_session_token, clean_stale_state_file}`, `onebrain_search::engine::Engine`. Output: text default, `--json`/`--yaml` structured (machine consumers must pass `--json`). The same file also implements `session token` (hidden, `run_token`) — a resolve-only counterpart for mid-session recovery that shares `resolve_token_or_override` with `session init` but skips vault resolution, config load, the search probe, and `clean_stale_state_file` entirely, emitting just `{"session_token":...}` / `token=<token>`.

### `src/commands/checkpoint.rs`
Implements `checkpoint stop` and `checkpoint reset`. Resolves the session token, then delegates to `onebrain_cache::{handle_stop, handle_reset}` against `std::env::temp_dir()` cache. `stop` writes checkpoint metadata to stdout; `reset` clears the cadence counter. `reset --session-token <TOKEN>` (hidden, mirroring `session init --session-token`) resets the state of an **already-resolved** token — sanitized via `SessionToken::sanitize`, never re-hashed — instead of re-deriving one from the environment: `onebrain hook` counts cadence under the token it derives from `ONEBRAIN_HOOK_SESSION_ID`, which the agent shell running `/wrapup` never sees, so an env-resolved reset zeroes a different token's state file and leaves the counter running. Library: `onebrain_cache`. Output: handled inside the cache crate (hook-protocol).
...
Implements `plugin install` (and the `register-hooks` alias). Idempotent `.claude/settings.json` wiring via `onebrain_fs::register_hooks::run(RegisterHooksOptions{vault_dir,dry_run,remove})` — adds one `onebrain hook` runner at Stop and, when `search.collection`/legacy `qmd_collection` is configured, one at PostToolUse; it also adds the OneBrain permission set. The generic runner reads the harness event from stdin and invokes checkpoint/reindex children itself, so direct `checkpoint stop` and `search reindex` remain user commands rather than registrations. Prints a summary; returns 0 / 1. Library: `onebrain_fs::register_hooks`.
...
During `plugin update`, converges CLI-managed Claude lifecycle entries to exactly one canonical `{ "type": "command", "command": "onebrain", "args": ["hook"] }` runner per existing Stop or PostToolUse event. It recognizes legacy checkpoint/pending, qmd, and search forms; removes stale managed lifecycle entries from unsupported events; deduplicates runners; and preserves foreign hooks plus unknown group/top-level fields. It never manufactures a missing PostToolUse registration because only hook registration knows whether search is configured, and it never adds a second Stop embed entry. `RewriteReport {total, converged, stale_entries_removed, warnings}` records real mutations; malformed entries are reported as `W_MALFORMED_HOOK_ENTRY` and preserved. `rewrite_settings_file(path, dry_run)` is byte-idempotent after the first pass.
**Connections** — called by: `plugin_update::run`. Tests cover legacy exec/shell forms, foreign-field preservation, stale cleanup, deduplication, dry-run, and second-pass idempotency.
