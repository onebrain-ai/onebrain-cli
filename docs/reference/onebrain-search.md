...
- `reranker(&self) -> Option<&dyn Rerank>`, `build_reranker`, `set_rerank_settings`, `set_rerank_min_candidates`, `rerank_enabled`, `rerank_active`, `set_reranker_for_tests` (`#[cfg(test)]`) — the `RerankSource`/`RerankSettings` plumbing mirroring `EmbedSource`'s lazy-construction pattern, with one added honesty rule: the lazy path resolves to `None` (skip, never fail) when the model isn't downloaded, the build lacks `semantic`, or construction fails. `set_rerank_min_candidates` is a lightweight override of just the `min_candidates` field (e.g. a per-query CLI flag / API param) that does NOT reset the lazy reranker source, unlike `set_rerank_settings`.
