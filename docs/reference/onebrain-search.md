...
- `LexIndex` — the index handle: schema fields `chunk_id` (`STRING | STORED`), `doc_path` (`STRING | STORED`), `heading_path` (`TEXT`, script-aware tokenizer, stored — same tokenizer as `body` as of v3.4.16, [ADR 0034](../decisions/0034-heading-search-schema-selfheal-rerank-gate-decouple.md); previously plain `TEXT` with tantivy's default English tokenizer, which is why upgrading requires the schema self-heal described under `open_or_reset` below), `body` (`TEXT`, script-aware tokenizer, `WithFreqsAndPositions`, not stored). The `IndexWriter` holds tantivy's **exclusive** directory lock, so it is created **lazily**: `open` acquires no writer lock; only the write paths materialize it on first use (`writer_mut`, 50 MB heap). Read-only opens therefore run concurrently with a writer — and past a stale lock left by a killed reindex.

**Key functions**
- `LexIndex::open(dir) -> Result<Self>` — open/create the index at `dir`, register the tokenizer; no writer lock. Fails with a schema-mismatch error (see `is_schema_mismatch` below) against an index built under the pre-v3.4.16 `heading_path` schema.
- `LexIndex::open_or_reset(dir) -> Result<(Self, bool)>` (v3.4.16, [ADR 0034](../decisions/0034-heading-search-schema-selfheal-rerank-gate-decouple.md)) — like `open`, but on a schema-mismatch error wipes and recreates the index at `dir` instead of propagating; the returned `bool` is `true` when a wipe+recreate happened (the caller, `Engine::open`, uses it to trigger `repopulate_lex_from_meta` from `chunk_meta`). Any other error (I/O, permissions) propagates untouched — this is the seam that makes the upgrade self-healing rather than a hard failure, without ever touching an index that's already current. Crash-safe: an interrupted reset is detected and retried on the next open rather than being mistaken for done or left half-populated.
- `is_schema_mismatch(err: &anyhow::Error) -> bool` (v3.4.16) — true when `err` downcasts to tantivy's typed `TantivyError::SchemaError`, matched on the enum variant rather than a message string so a wording change upstream can't silently turn the self-heal into a hard failure (or vice versa).
...
- `search(&self, query, top_k) -> Result<Vec<(String, f32)>>` — BM25 over `body` **and** `heading_path` (the heading side boosted at `HEADING_BOOST = 0.35`, v3.4.16 — see `build_query` below), highest score first. Deliberately bypasses tantivy's `QueryParser` (which pre-tokenizes as English): the query is segmented with the same script-aware routine, then each **no-space-script run** (a pseudo-word like `สุขภาพ`) becomes a nested `BooleanQuery` requiring **all** of its bigrams (`with_minimum_required_clauses = n`) — exact substring-style semantics, so a hit means the document really contains the queried word, while a substring query (`ภาพ`) still matches any doc containing it; each token of an **other** run becomes one `Should` `TermQuery`. Multi-word Thai should be spaced (OR of runs); fuzzy recall is the vector side's job. Guards `top_k == 0` (tantivy's `TopDocs::with_limit` panics on 0).
- `search_with_heading(&self, query, top_k) -> Result<Vec<(String, String, f32)>>` — like `search`, but also returns each hit's `heading_path`, read from the same retrieved document as `chunk_id` (both are `STORED`) — no second lookup. Used by the CLI's `search search` verb so it can display a heading without opening `engine.redb`.
- `build_query(&self, query: &str) -> Option<BooleanQuery>` (private, v3.4.16 rewrite) — builds every query unit **twice**, once against `body` and once against `heading_path` wrapped in `BoostQuery(HEADING_BOOST)`, both `Should`-combined so a chunk matching either field is retrievable and one matching both scores higher. Returns `None` for a query with no searchable units (empty/punctuation-only).

**Connections** — calls: `tantivy`, `crate::chunk::Chunk`; called by: `engine::{index_doc, remove_doc, query}`, and (`open_or_reset`) `engine::Engine::open`.
**Tests** — English BM25, bigram matches in Thai/Chinese/Japanese/Korean/Lao, Russian via the default path, delete, whole-Thai-word-required vs substring semantics, empty/punctuation queries, `top_k == 0`, the lazy-writer contract (concurrent read-only opens, stale `.tantivy-writer.lock` tolerated, `search` never materializes a writer), heading-field matches (Latin and script-aware), `HEADING_BOOST` ranking relative to a body-only match, and `open_or_reset` self-heal (wipes + reports `true` on a legacy schema, leaves a current-schema index untouched and reports `false`).
...
- `Hit` — a fused, resolved result: `{ chunk_id, doc_path, heading_path, score, snippet, rerank_score }` (snippet char-boundary-truncated to 200 chars + `…` — safe for multibyte Thai). `rerank_score: Option<f32>` is the Tier-2 cross-encoder's calibrated 0–1 score; `None` means the hit is unreranked (stage skipped or failed — the reranked pool is `max(min_candidates, top_k)`, so every returned hit is normally reranked; nothing beyond that pool survives truncation to `top_k`).
...
- `query(&self, text, top_k) -> Result<Vec<Hit>>` — hybrid search: `embed_query` → vector top-50 trimmed to the top cluster (`keep_top_cluster`, within `VEC_CLUSTER_WINDOW` of the best score — recall-first, relative) + lex top-50 → `rrf_fuse(.., RRF_K, fuse_k)` (`fuse_k = max(top_k, rerank_settings.min_candidates)`, wide enough for the rerank stage) → `apply_rerank` (Tier-2 cross-encoder stage, [ADR 0025](../decisions/0025-tier2-cross-encoder-reranker.md)) → `resolve_hits`.
- `vector_search(&self, text, top_k)` — vector-only semantic search (no lex/RRF fusion), fetched at `max(top_k, min_candidates)` and passed through the same `apply_rerank` stage; the CLI's `search vsearch` verb.
- `apply_rerank(&self, query, fused, top_k) -> Result<Vec<Hit>>` (private) — the Tier-2 rerank stage shared by `query` and `vector_search`: split the fused/retrieved list into a reranked pool (`max(min_candidates, top_k)`-sized — `min_candidates` is a FLOOR, auto-raised to cover `top_k` so every returned result is always reranked) and tail; no reranker resolved → return the fused order unreranked; else cross-encode the pool's chunk texts (`chunk_texts`) against `query`, sort by calibrated score descending (chunk-id tiebreak), gate at `rerank_settings.min_score` (`DEFAULT_RERANK_MIN_SCORE = 0.0` as of v3.4.16, [ADR 0034](../decisions/0034-heading-search-schema-selfheal-rerank-gate-decouple.md) — the gate drops nothing by default, so this step normally re-**sorts** the pool rather than shrinking it; raising `min_score` opts back into filtering, with a never-empty floor `RERANK_NO_MATCH_KEEP = 3` guarding total rejection), append the untouched tail with `rerank_score: None`, truncate to `top_k` (which now always drops that tail, since the pool already covers the returned set), resolve. A rerank call error degrades the same way as no-reranker (skip-not-fail) after a once-per-engine stderr log.
...
- `reranker(&self) -> Option<&dyn Rerank>`, `build_reranker`, `set_rerank_settings`, `set_rerank_min_candidates`, `set_rerank_min_score`, `rerank_enabled`, `rerank_active`, `set_reranker_for_tests` (`#[cfg(test)]`) — the `RerankSource`/`RerankSettings` plumbing mirroring `EmbedSource`'s lazy-construction pattern, with one added honesty rule: the lazy path resolves to `None` (skip, never fail) when the model isn't downloaded, the build lacks `semantic`, or construction fails. `set_rerank_min_candidates` and `set_rerank_min_score` are lightweight overrides of just those fields (a per-query CLI flag / API param) that do NOT reset the lazy reranker source, unlike `set_rerank_settings`. `set_rerank_min_score` backs the CLI `--min-score` unification: when reranking is active the flag overrides this gate (filtering the calibrated 0–1 `rerank_score`) instead of the raw retrieval score.
