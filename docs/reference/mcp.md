...
`context` is omitted entirely (not `null`) when the hit has no heading path. `rerank_score` (calibrated 0–1, from the Tier-2 cross-encoder) carries the confidence of each returned hit: after fusing the sub-queries, `query` reranks the whole fused pool against the **primary** (first, 2×-weighted) sub-query's text — so every returned hit, regardless of which sub-query surfaced it, carries a `rerank_score`. It is omitted entirely — not `null` — only when the rerank stage was skipped for the whole query: reranking disabled, the model not downloaded, or a load/runtime failure (skip-not-fail keeps the fused RRF order). A missing key and an unreranked hit are the same thing; there is no separate boolean flag for "was this reranked."
...
- **Tools** (the "Brain pack"):
  ...
  |---|---|---|
  | `capabilities` | read-only | Self-description — packs, vaults, default vault, each tool's risk class + effective policy mode, and which approval channels (`native`/`http`/`telegram`) can actually deliver a prompt on this machine right now — `telegram` reports whether `gateway.yml` has a bot configured (`onebrain gateway telegram setup`), not whether it's currently reachable. See [`docs/gateway.md#capabilities-truthfulness`](../gateway.md#capabilities-truthfulness). |
  ...
  | `brain_capture` | mutating | Create a new inbox note from `title`/`text` — the gateway's first WRITE tool. Gated by [policy](../gateway.md#policy--approvals): defaults to needing one human approval per client, per vault, before it proceeds, then remembers that consent for `grant_ttl_minutes`. See [`docs/gateway.md#brain_capture`](../gateway.md#brain_capture). |

- **Policy, approvals, and an audit trail.** Every tool call is classified `read_only`/`mutating`/`destructive` and checked against a per-class mode (`auto`/`ask_once`/`ask_always`/`deny`, configured under `gateway.yml`'s `policy:` block). A call that needs approval blocks until a human answers — via a native macOS dialog, the operator `GET`/`POST /approvals` HTTP surface (gated by the gateway's pairing code, never by a connector's own bearer token), and/or a dedicated Telegram bot with inline Approve/Deny buttons (`onebrain gateway telegram setup`; its own auth model has no pairing code at all — see [`docs/gateway.md#telegram-approval-channel`](../gateway.md#telegram-approval-channel)) — or times out. Every call, allowed or not, is appended to `~/.onebrain/gateway/audit/YYYY-MM.jsonl`, whose `channel` field names which channel resolved a human decision (`null` when none was ever consulted: `auto`, a policy `denied` with no human involved, or a timeout). Full details at [`docs/gateway.md#policy--approvals`](../gateway.md#policy--approvals).
