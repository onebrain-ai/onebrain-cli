...
- **MCP protocol version**: the server speaks MCP protocol `2026-07-28` (rmcp 3.0.1) and negotiates down to the version a client requests at `initialize` — `2025-11-25`, `2025-03-26`, and earlier eras keep working unchanged (guarded by `tests/mcp_stdio.rs::initialize_negotiates_the_client_requested_protocol_version`). The server *implementation* version (reported in `initialize`'s `serverInfo`) still always matches `onebrain --version`.
