...
- [x] **v3.2.3** — `skill run` fixes (walk-up from cwd so it runs inside a vault without an explicit path · null stdin so `claude -p` can't hang on an interactive TTY) · global `--vault` now accepted on every command (`skill run` / `schedule register` / `plugin migrate` had a `--vault-dir` field-name collision suppressing it) · `doctor` stamps `stats.last_doctor_run` (+ `last_doctor_fix` on `--fix`).
