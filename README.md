...
- [ ] **v3.4** — **Native Search + Warm Daemon** *(in progress — completes the foundation)*: native Rust search replaces qmd (native engine + `onebrain search` verbs · native MCP server · 0 node/python deps, v3.4.5), plus a minimal **warm daemon** that owns the engine for mcp + search so the CLI works during a live session and multiple sessions coexist (v3.4.6), plus a Tier-2 cross-encoder reranker (`onebrain-rerank-v1`) for calibrated result relevance (v3.4.7) ([milestone](https://github.com/onebrain-ai/onebrain-cli/milestone/4)).
