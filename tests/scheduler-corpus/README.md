...
| `accept-generated-escaping.{service,timer,xml}` + `.expect` | **The escapers, through the real tools.** Until v3.4.22 no fixture carried a `$`, `%`, quote, space, `;` or backslash, so `systemd-analyze verify` and `schtasks /Create` ran on every PR without exercising either escaper (#353). Measured while adding it: systemd RAN the unit and `/bin/echo` received `$HOME` **unexpanded** — the point of `$` → `$$`, and the one thing the *static* verify cannot tell you. Task Scheduler reported back `"C:\My Vault\\"`, the trailing backslash run doubled. The argument list is generated, so its reason lives in `test_support::escaping_args` rather than inside the artifact: each element names the branch it reaches, and the claim that they cover every branch is a `cargo llvm-cov` measurement — an earlier version of that same claim was false. |
...
| `accept-generated-*.service` `Environment="ONEBRAIN_VAULT=…"` | The ownership marker (#410): every unit names the vault that installed it so `register` can prune by ownership, never by name. `Environment=` expands `%` specifiers (hence `%%`) but not `$` — the opposite half of the `ExecStart` rules `quote_arg` handles. Task Scheduler carries the same marker in `<Description>`; launchd in `EnvironmentVariables`. |
